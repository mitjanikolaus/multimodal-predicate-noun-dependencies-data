...
def eval_set_stats(args):
    ...
    plt.xlabel("")
    plt.tight_layout()
    plt.savefig("results_figures/num_samples_per_word.pdf", dpi=300)

    plt.figure(figsize=(5, 6))
    data_nouns = eval_set_per_word[~eval_set_per_word.word.isin(OBJECTS_VERBS)]
    g = data_nouns.groupby("word").size().plot.barh(color="black")
    ...
    plt.ylabel("")
    plt.tight_layout()
    plt.savefig("results_figures/num_samples_per_word_nouns.pdf", dpi=300)

    plt.figure(figsize=(5, 3))
    data_verbs = eval_set_per_word[eval_set_per_word.word.isin(OBJECTS_VERBS)]
    g = data_verbs.groupby("word").size().plot.barh(color="black")
    g.set_xscale("log")
    plt.xlabel("# Samples")
    plt.ylabel("")
    plt.tight_layout()
    plt.savefig("results_figures/num_samples_per_word_verbs.pdf", dpi=300)
